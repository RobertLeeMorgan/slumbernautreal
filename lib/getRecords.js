...
export const getRecords = cache(async () => {
  try {
    ...
    const records = items.map((e) => {
      ...
      let link = `https://distrokid.com/hyperfollow/${process.env.NEXT_PUBLIC_ARTIST_NAME}/${formattedName}`;
      if (releaseDate < new Date('2021-01-01')) {
        link = `https://hypeddit.com/${process.env.NEXT_PUBLIC_ARTIST_NAME}/${formattedName}`;
      } else if (e.name === 'Taste of Spring (Edit)') {
        link = `https://distrokid.com/hyperfollow/${process.env.NEXT_PUBLIC_ARTIST_NAME}/taste-of-spring-edit`;
      }
